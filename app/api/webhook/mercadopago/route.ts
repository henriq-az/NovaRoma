...
export async function POST(request: Request) {
  ...
  if (novoStatus === 'aprovado') {
    ...
    if (itens) {
      ...
      const emailCliente = clienteData.email_cliente as string | undefined
      if (emailCliente) {
        try {
          ...
          const total = (itensCompletos ?? []).reduce(
            (acc: number, i: { quantidade: number; preco_unit: number }) => acc + i.quantidade * i.preco_unit,
            0
          )
